...
//		List<User> user = (List<User>) hibernateTemplate.findByNamedParam("select u from User where u.phone = :ph and u.password = :ps", new String[] {"ph" , "ps"}, new Object[] {phone, password});
		List<User> user = (List<User>) hibernateTemplate.find("select u from User u where u.phone = ?0 and u.password = ?1", phone, password);
	...
	public User verifyUserByEmailAndPassword(String email, String password) {
		...
		List<User> user = (List<User>) hibernateTemplate.find("select u from User u where u.email = ?0 and u.password = ?1", email, password);
	...
	public User verifyUserByIdAndPassword(int id, String password) {
		...
		List<User> user = (List<User>) hibernateTemplate.find("select u from User u where u.id = ?0 and u.password = ?1", id, password);
